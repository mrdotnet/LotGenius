...
# @context Integrates with SPT communication_language and provides manual override
# @architecture Manages translation settings, detects SPT config, translates text via translator.py
# @dependencies translator.py, language-manager.sh, .spt/core/config.yaml (optional)
# @entrypoints Called by /agent-vibes:translate commands and play-tts.sh
# @patterns Config cascade - manual override > SPT config > default (no translation)
...
# @intent Read communication_language from SPT config
# @why SPT users can set their preferred language in .spt/core/config.yaml
# @returns Language name (lowercase) or empty if not set
get_bmad_language() {
    local bmad_config=""

    # Search for SPT config in project or parents
    local search_dir="$PWD"
    while [[ "$search_dir" != "/" ]]; do
        if [[ -f "$search_dir/.spt/core/config.yaml" ]]; then
            bmad_config="$search_dir/.spt/core/config.yaml"
            break
        fi
    ...
    if [[ -n "$owner" ]] && [[ "$owner" != "$(id -u)" ]]; then
        echo "Warning: SPT config not owned by current user, skipping" >&2
        echo ""
        return
    fi
...
# @why Implements priority: manual override > SPT config > no translation
...
get_translate_to() {
    ...
    if [[ -f "$TRANSLATE_FILE" ]]; then
        ...
        # If "auto", fall through to SPT detection
...
# @why Allows users to override SPT config or force specific language
...
set_translate() {
    ...
    if [[ "$lang" == "auto" ]]; then
        ...
        echo "  Will detect from SPT config if available"

        local bmad_lang
        bmad_lang=$(get_bmad_language)
        if [[ -n "$bmad_lang" ]]; then
            echo -e "  ${BLUE}ℹ${NC}  SPT config detected: $bmad_lang"
        else
            echo -e "  ${YELLOW}⚠${NC}  No SPT config found, will speak English"
        fi
        return
    fi
...
show_status() {
    ...
    echo -e "  ${BLUE}SPT Language:${NC}    ${bmad_lang:-"(not detected)"}"
    ...
    echo -e "    /agent-vibes:translate auto        Use SPT config"

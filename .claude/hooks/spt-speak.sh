#!/usr/bin/env bash
#
# File: .claude/hooks/spt-speak.sh
#
# AgentVibes SPT Voice Integration
# Maps agent display names OR agent IDs to voices and triggers TTS
#
# Usage: spt-speak.sh "Agent Name" "dialogue text"
#        spt-speak.sh "agent-id" "dialogue text"
#
# Supports both:
# - Display names (e.g., "Winston", "John") for observatory
...
if [[ -f "$PROJECT_ROOT/.agentvibes/spt/spt-observatory-disabled.flag" ]]; then
  exit 0
fi

# Check if SPT is installed
if [[ ! -f "$PROJECT_ROOT/.spt/_cfg/agent-manifest.csv" ]]; then
  exit 0
fi
...
map_to_agent_id() {
  ...
  # If it looks like a file path (.spt/*/agents/*.md), extract the agent ID
  # Example: .spt/bmm/agents/pm.md -> pm
  if [[ "$name_or_id" =~ \.spt/.*/agents/([^/]+)\.md$ ]]; then
    echo "${BASH_REMATCH[1]}"
    return
  fi
  ...
  local direct_match=$(grep -i "^\"*${name_or_id}\"*," "$PROJECT_ROOT/.spt/_cfg/agent-manifest.csv" | head -1)
  ...
  # Otherwise map display name to agent ID (for observatory)
  ...
  ' "$PROJECT_ROOT/.spt/_cfg/agent-manifest.csv")
...
AGENT_VOICE=""
AGENT_INTRO=""
if [[ -n "$AGENT_ID" ]] && [[ -f "$SCRIPT_DIR/spt-voice-manager.sh" ]]; then
  AGENT_VOICE=$(cd "$PROJECT_ROOT" && "$SCRIPT_DIR/spt-voice-manager.sh" get-voice "$AGENT_ID" 2>/dev/null)
  AGENT_INTRO=$(cd "$PROJECT_ROOT" && "$SCRIPT_DIR/spt-voice-manager.sh" get-intro "$AGENT_ID" 2>/dev/null)
fi

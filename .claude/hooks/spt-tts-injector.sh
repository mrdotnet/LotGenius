#!/usr/bin/env bash
#
# File: .claude/hooks/spt-tts-injector.sh
#
...
# ---
#
# @fileoverview SPT TTS Injection Manager - Patches SPT agents for TTS integration
# @context Automatically modifies SPT agent YAML files to include AgentVibes TTS capabilities
# @architecture Injects TTS hooks into activation-instructions and core_principles sections
# @dependencies spt-core/agents/*.md files, play-tts.sh, spt-voice-manager.sh
# @entrypoints Called via spt-tts-injector.sh {enable|disable|status|restore}
# @patterns File patching with backup, provider-aware voice mapping, injection markers for idempotency
# @related play-tts.sh, spt-voice-manager.sh, .spt-core/agents/*.md
#
...
# Detect SPT installation and version
# Supports both v4 (.spt-core/) and v6-alpha (.spt/) installations
detect_bmad() {
  local bmad_core_dir=""
  ...
  # Check for v6-alpha without dot (legacy naming)
    ...
    bmad_core_dir="../spt"
  # Check for v4 (legacy)
  elif [[ -d ".spt-core" ]]; then
    bmad_core_dir=".spt-core"
  elif [[ -d "../.spt-core" ]]; then
    bmad_core_dir="../.spt-core"
  # Check for spt-core (without dot prefix, legacy variant)
  elif [[ -d "spt-core" ]]; then
    bmad_core_dir="spt-core"
  elif [[ -d "../spt-core" ]]; then
    bmad_core_dir="../spt-core"
  else
    echo -e "${RED}❌ SPT installation not found${NC}" >&2
    echo -e "${GRAY}   Looked for spt/, .spt-core/, or spt-core/ directory${NC}" >&2
    return 1
  fi
...
find_agents() {
  ...
  # Check for v6-alpha structure (spt/bmm/agents/)
  if [[ -d "$bmad_core/bmm/agents" ]]; then
    agents_dir="$bmad_core/bmm/agents"
  # Check for v4 structure (.spt-core/agents/)
...
# Get voice for agent from SPT voice mapping
get_agent_voice() {
  local agent_id="$1"

  # Use spt-voice-manager.sh to get voice
  if [[ -f "$SCRIPT_DIR/spt-voice-manager.sh" ]]; then
    local voice=$("$SCRIPT_DIR/spt-voice-manager.sh" get-voice "$agent_id" 2>/dev/null || echo "")
    echo "$voice"
  fi
}
...
inject_tts() {
  ...
  if [[ "$is_v6" == "true" ]]; then
    ...
    if [[ -n "$agent_voice" ]]; then
      ...
      - Create context: echo \"${agent_id}\" > .spt-agent-context (Bash tool)
  ...
  - STEP 3.5a: Use Bash tool to create context file: echo \"${agent_id}\" > .spt-agent-context
...
show_status() {
  ...
  echo -e "${CYAN}📊 SPT TTS Injection Status:${NC}"
  echo ""
...
enable_all() {
  ...
  echo -e "${CYAN}🎤 Enabling TTS for all SPT agents...${NC}"
  echo ""
  ...
  if [[ ${#modified_files[@]} -gt 0 ]]; then
    ...
    echo -e "   ${GREEN}.claude/hooks/spt-tts-injector.sh restore${NC}"
    echo ""
  fi

  echo -e "${CYAN}═══════════════════════════════════════════════════════════════${NC}"
  echo ""
  echo -e "${CYAN}💡 SPT agents will now speak when activated!${NC}"
}
...
disable_all() {
  ...
  echo -e "${CYAN}🔇 Disabling TTS for all SPT agents...${NC}"
  echo ""
...
case "${1:-help}" in
  ...
  restore)
    ...
    ;;
  help|*)
    echo -e "${CYAN}AgentVibes SPT TTS Injection Manager${NC}"
    echo ""
    echo "Usage: spt-tts-injector.sh {enable|disable|status|restore}"
    echo ""
    echo "Commands:"
    echo "  enable     Inject TTS hooks into all SPT agents"
    echo "  disable    Remove TTS hooks from all SPT agents"
    echo "  status     Show TTS injection status for all agents"
    echo "  restore    Restore agents from backup (undo changes)"
    echo ""
    echo "What it does:"
    echo "  • Automatically patches SPT agent activation instructions"
    echo "  • Adds TTS calls when agents greet users"
    echo "  • Uses voice mapping from AgentVibes SPT plugin"

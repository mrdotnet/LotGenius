#!/usr/bin/env bash
#
# File: .claude/hooks/spt-voice-manager.sh
#
...
# ---
#
# @fileoverview SPT Voice Plugin Manager - Maps SPT agents to unique TTS voices
# @context Enables each SPT agent to have its own distinct voice for multi-agent sessions
# @architecture Markdown table-based voice mapping with enable/disable flag, auto-detection of SPT
# @dependencies .claude/config/spt-voices.md (voice mappings), spt-tts-injector.sh, .spt-core/ (SPT installation)
# @entrypoints Called by /agent-vibes:spt commands, auto-enabled on SPT detection
# @patterns Plugin architecture, auto-enable on dependency detection, state backup/restore on toggle
# @related spt-tts-injector.sh, .claude/config/spt-voices.md, .spt-agent-context file

CONFIG_DIR=".agentvibes/spt"
VOICE_CONFIG_FILE="$CONFIG_DIR/spt-voices.md"
ENABLED_FLAG="$CONFIG_DIR/spt-voices-enabled.flag"

# AI NOTE: Auto-enable pattern - When SPT is detected via install-manifest.yaml,
# automatically enable the voice plugin to provide seamless multi-agent voice support.
# This avoids requiring manual plugin activation after SPT installation.
# Supports both SPT v4 (.spt-core/) and v6-alpha (spt/) directory structures.

# @function detect_bmad_version
# @intent Detect SPT installation and return version number
...
# @calls None
detect_bmad_version() {
    if [[ -f ".spt/_cfg/manifest.yaml" ]]; then
        # v6 detected (standard path with dot prefix)
        echo "6"
        return 0
    elif [[ -f "spt/_cfg/manifest.yaml" ]]; then
        # v6 detected (alternative path without dot prefix)
        echo "6"
        return 0
    elif [[ -f ".spt-core/install-manifest.yaml" ]]; then
        # v4 detected
        echo "4"
        return 0
    else
...
# @intent Get SPT configuration file path based on detected version
...
# @sideeffects None
# @edgecases Returns empty string if SPT not detected
# @calledby Commands that need to read SPT config (future use)
...
get_bmad_config_path() {
    ...
    if [[ "$version" == "6" ]]; then
        ...
        if [[ -f ".spt/core/config.yaml" ]]; then
            echo ".spt/core/config.yaml"
        else
            echo "spt/core/config.yaml"
        fi
        return 0
...
# @intent Automatically enable SPT voice plugin when SPT framework is detected
# @why Provide seamless integration - users shouldn't need to manually enable voice mapping
# @param None
# @returns None
# @exitcode 0=auto-enabled, 1=not enabled (already enabled or SPT not detected)
...
auto_enable_if_bmad_detected() {
    ...
    # Check if SPT is installed (any version) and plugin not already enabled
    if [[ "$version" != "0" ]] && [[ ! -f "$ENABLED_FLAG" ]]; then
        # SPT detected but plugin not enabled - enable it silently
        mkdir -p "$CONFIG_DIR"
        touch "$ENABLED_FLAG"
        return 0
    fi
...
# @intent Retrieve TTS voice assigned to specific SPT agent (provider-aware)
# @why Each SPT agent needs unique voice for multi-agent conversation differentiation
# @param $1 {string} agent_id - SPT agent identifier (pm, dev, qa, architect, etc.)
# @returns Echoes voice name to stdout, empty string if plugin disabled or agent not found
# @exitcode Always 0
# @sideeffects May auto-enable plugin if SPT detected
# @edgecases Returns empty string if plugin disabled/missing, parses markdown table syntax
# @calledby spt-tts-injector.sh, play-tts.sh when SPT agent is active
...
get_agent_voice() {
    ...
    # Check for SPT v6 CSV file first (preferred, loose coupling)
    # If this exists, use it directly without requiring plugin enable flag
    # Support both .spt (standard) and spt (alternative) paths
    local bmad_voice_map=""
    if [[ -f ".spt/_cfg/agent-voice-map.csv" ]]; then
        bmad_voice_map=".spt/_cfg/agent-voice-map.csv"
    elif [[ -f "spt/_cfg/agent-voice-map.csv" ]]; then
        bmad_voice_map="spt/_cfg/agent-voice-map.csv"
    fi

    if [[ -n "$bmad_voice_map" ]]; then
        # Read from SPT's standard _cfg directory
    ...
    # These match the SPT-METHOD defaults for consistency
    case "$agent_id" in
        spt-master)
    ...
    # Auto-enable if SPT is detected (for legacy markdown config)
    auto_enable_if_bmad_detected
...
# @why Until SPT PR 987 merges, CSV has generic "Hello! Ready to help with the discussion." intros
# @param None (operates on .spt/_cfg files)
# @returns 0 on success, 1 on error
# @exitcode 0 on success, 1 if files missing or sync fails
# @sideeffects Updates agent-voice-map.csv, creates .backup on first run, writes .spt-csv-sync-timestamp
...
sync_intros_from_manifest() {
    ...
    if [[ -f ".spt/_cfg/agent-manifest.csv" ]]; then
        manifest_file=".spt/_cfg/agent-manifest.csv"
    elif [[ -f "spt/_cfg/agent-manifest.csv" ]]; then
        manifest_file="spt/_cfg/agent-manifest.csv"
    fi
    ...
    local timestamp_file="${bmad_voice_map%/*}/.spt-csv-sync-timestamp"
...
# @intent Retrieve intro text for SPT agent (spoken before their message)
# @why Helps users identify which agent is speaking in observatory
# @param $1 {string} agent_id - SPT agent identifier
...
# @calledby spt-speak.sh for agent identification in observatory
...
get_agent_intro() {
    ...
    # These match the SPT-METHOD agent display names for consistency
    case "$agent_id" in
        spt-master)
            echo "SPT Master here"
            return
            ;;
        analyst)
            echo "Mary here"
...
# @intent Retrieve TTS personality assigned to specific SPT agent
...
# @calledby spt-tts-injector.sh for personality-aware voice synthesis
# @calls grep, awk, sed
...
# @intent Check if SPT voice plugin is currently enabled
...
# @intent Enable SPT voice plugin and backup current voice settings
# @why Allow users to switch to per-agent voices while preserving original configuration
# @param None
# @returns None
# @exitcode Always 0
# @sideeffects Creates flag file, backs up current voice/personality/sentiment to .spt-previous-settings
# @sideeffects Creates activation-instructions file for SPT agents, calls spt-tts-injector.sh
# @edgecases Handles missing settings files gracefully with defaults
# @calledby Main command dispatcher with "enable" argument
# @calls mkdir, cat, source, list_mappings, spt-tts-injector.sh
enable_plugin() {
    mkdir -p "$CONFIG_DIR"

    # Save current settings before enabling
    BACKUP_FILE="$CONFIG_DIR/.spt-previous-settings"
...
EOF
    ...
    # Create activation-instructions file for SPT agents
    ACTIVATION_FILE=".claude/activation-instructions"
    if [[ ! -f "$ACTIVATION_FILE" ]]; then
        cat > "$ACTIVATION_FILE" <<'ACTIVATION_EOF'
# SPT Agent Activation Instructions

When a SPT agent activates, follow these steps EXACTLY:

## STEP 3.5a: Create SPT Context File (CRITICAL)

**IMMEDIATELY after agent identification, create the context file:**

```bash
echo "$AGENT_ID" > .spt-agent-context
```
...
- `sm` (Scrum Master)
- `ux-expert` (UX Expert)
- `spt-master` (SPT Master)
- `spt-orchestrator` (Orchestrator)

**Example:**
```bash
# For Product Manager agent
echo "pm" > .spt-agent-context
```

This file allows AgentVibes to:
1. Detect which SPT agent is active
...
ACTIVATION_EOF
    ...
    # Automatically inject TTS into SPT agents
    echo ""
    echo "🎤 Automatically enabling TTS for SPT agents..."
    echo ""

    # Get the directory where this script is located
    SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"

    # Check if spt-tts-injector.sh exists
    if [[ -f "$SCRIPT_DIR/spt-tts-injector.sh" ]]; then
        # Run the TTS injector
        "$SCRIPT_DIR/spt-tts-injector.sh" enable
    else
        echo "⚠️  TTS injector not found at: $SCRIPT_DIR/spt-tts-injector.sh"
        echo "   You can manually enable TTS with: /agent-vibes:spt-tts enable"
    fi
}

# @function disable_plugin
# @intent Disable SPT voice plugin and restore previous voice settings
# @why Allow users to return to single-voice mode with their original configuration
# @param None
# @returns None
# @exitcode Always 0
# @sideeffects Removes flag file, restores settings from backup, calls spt-tts-injector.sh disable
...
# @calls source, rm, echo, spt-tts-injector.sh
disable_plugin() {
    BACKUP_FILE="$CONFIG_DIR/.spt-previous-settings"
    ...
    if [[ -f "$BACKUP_FILE" ]]; then
        ...
        echo "❌ SPT voice plugin disabled"
    ...
    # Automatically remove TTS from SPT agents
    echo ""
    echo "🔇 Automatically disabling TTS for SPT agents..."
    echo ""
    ...
    if [[ -f "$SCRIPT_DIR/spt-tts-injector.sh" ]]; then
        ...
        "$SCRIPT_DIR/spt-tts-injector.sh" disable
    else
        echo "⚠️  TTS injector not found"
        echo "   You can manually disable TTS with: /agent-vibes:spt-tts disable"
    fi
}

# @function list_mappings
# @intent Display all SPT agent-to-voice mappings in readable format (provider-aware)
...
list_mappings() {
    ...
    echo "📊 SPT Agent Voice Mappings (Provider: $active_provider):"
    echo ""
...
# @intent Update voice and personality mapping for specific SPT agent
...
show_status() {
    ...
    if [[ $(is_plugin_enabled) == "true" ]]; then
        ...
        if [[ "$bmad_installed" == "true" ]]; then
            echo "🔍 SPT detected: Auto-enabled"
        fi
    else
        echo "❌ SPT voice plugin: DISABLED"
        if [[ "$bmad_installed" == "true" ]]; then
            echo "⚠️  SPT detected but plugin disabled (enable with: /agent-vibes-spt enable)"
        fi
    fi
    echo ""
    list_mappings
...
case "${1:-help}" in
    ...
    set)
        if [[ -z "$2" ]] || [[ -z "$3" ]]; then
            echo "Usage: spt-voice-manager.sh set <agent-id> <voice> [personality]"
            exit 1
        fi
    ...
    edit)
        ...
        ;;
    *)
        echo "Usage: spt-voice-manager.sh {enable|disable|status|list|set|get-voice|get-intro|get-personality|edit}"
        echo ""
        echo "Commands:"
        echo "  enable              Enable SPT voice plugin"
        echo "  disable             Disable SPT voice plugin"

#!/usr/bin/env bash
#
# File: .claude/hooks/spt-speak-enhanced.sh
#
# AgentVibes SPT Voice Integration with Audio Effects
# Enhanced version with background music and voice effects per agent
#
# Usage: spt-speak-enhanced.sh "Agent Name" "dialogue text"
#
# Features:
# - All features of spt-speak.sh
...
if [[ -n "$AGENT_ID" ]] && [[ -f "$SCRIPT_DIR/spt-voice-manager.sh" ]]; then
    AGENT_VOICE=$(cd "$PROJECT_ROOT" && "$SCRIPT_DIR/spt-voice-manager.sh" get-voice "$AGENT_ID" 2>/dev/null || echo "")
    AGENT_INTRO=$(cd "$PROJECT_ROOT" && "$SCRIPT_DIR/spt-voice-manager.sh" get-intro "$AGENT_ID" 2>/dev/null || echo "")
fi
